...
const MovieList = (props: { reduxState: MovieListState; singleState: MovieSingle }) => {
  ...
  }, [scrollTop, count, fetchingMovie, isLoading, search, movies.totalResults, setCounter]);
  ...
  const movieList = () => {
    ...
    if (!isLoading && search.trim().length > 0 && movies.Search && movies.Search.length > 0) {
      ...
      const isSeeAble = count > 1 && scrollTop + window.innerHeight > 1000;
      return (
